...
inputext = 'png'
outputdir = '/media/water/2016/PROCESSED_DATA/Jamaica201607/r20160709_151200_pr16_region3_dense_ns/i20160709_151200_clahe2'
...
def colorCLAHE(imagename,outputdir,clahe_arg):
    img = cv2.imread(imagename,1) # assumes colour image
    cv2.imshow("img",img)
    ...
    outfile = os.path.join(outputdir, os.path.basename(imagename))
    cv2.imwrite(outfile, final)

num_cores = multiprocessing.cpu_count()

Parallel(n_jobs=num_cores)(delayed(colorCLAHE)(file,outputdir,clahe) for file in glob.glob(inputpath))
