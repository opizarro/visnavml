...
#camera_poses = 'loki_nav_'+dive_name+'.csv'
    ...
    python_miniscript = os.path.join(workfolder,'start_photoscan_proc_'+dive_name+'.sh')
    ...
    with open(python_miniscript, 'w+') as f:
        ...
        f.write("#!/bin/sh\n")
        f.write("PROC_SCRIPT=%s\n" % photoscan_steps_script)
        f.write("CONFIG_FILE=%s\n" % ps_config_file_name)
        f.write("/home/opizarro/Downloads/photoscan-pro_1.3.2/photoscan.sh -r $PROC_SCRIPT $CONFIG_FILE ")
        #f.write("config_file = '%s' \n" % ps_config_file_name)
